...
# The node split job
# ########################################
def NodeSplit( nodeDepth, nodeDirectory, inmem_queue, outmem_queue, THRESHOLDFILE, RAMDISK):
	...
	# if the data is still there, run the split
		...
		os.mkdir(child0)
		os.mkdir(child1)
		# if the feature file is larger than the ramdisk size
		if os.path.getsize(featureFile) > 1024*1024*1024 :
			...
			# do some symlinking for the data of the childs
			ramdiskdir0 = RAMDISK + "/" + child0.replace("/","_")
			ramdiskdir1 = RAMDISK + "/" + child1.replace("/","_")
			os.mkdir(ramdiskdir0)
			os.mkdir(ramdiskdir1)
			os.symlink(ramdiskdir0, child0+"/data")
			os.symlink(ramdiskdir1, child1+"/data")
			# run_SplitSamples to feature files in these dirs -> *.feat file 
			run_splitSamples( splitInfoFile, featureFile, child0+"/data/features.feat", child1+"/data/features.feat")
			# reverse order append cause its a stack
			inmem_queue.append((nodeDepth+1, child1))
			inmem_queue.append((nodeDepth+1, child0))
	...
	# uglyness to remove the data dir... if rmdir fails.. we have a symlink
	try:
		os.rmdir(dataDirectory)
	except :
		realdir = os.readlink(dataDirectory)
		os.rmdir(realdir)
...
# ########################################
def developTree( baseDir, maxDepth, featureFile0, thresholdFile, RAMDISK):

	for f in glob.glob( RAMDISK + "/*"): shutil.rmtree(f)
	...
	while ( OUTMEM_jobs or INMEM_jobs ):
		...
		while ( INMEM_jobs ) :
			...
			if nodeDepth <= maxDepth: NodeSplit( nodeDepth, nodeDirectory, INMEM_jobs, OUTMEM_jobs, thresholdFile, RAMDISK)
		if( OUTMEM_jobs ) :
...
parser.add_argument('-ramdisk',       type=str)
...
# go go go
developTree( args.baseDir, args.maxDepth,  args.featureFile0, args.thresholdFile, args.ramdisk )
