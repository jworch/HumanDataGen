                         ...
                         std::vector<int>&            vjs_bounds )  // NVertices+1 long, vjs_bounds[i] is begin offset for vertex i, vjs_bounds[i+1] is end offset
{
	...
	for(int vi=0;vi<numVertices;++vi) {
		...
		while( 1 ) {
			...
			float weight;
			iss >> name >> weight;
			if( iss.fail() ) break; // we hit the end of the line
		...
		if( int(vjs.size()) == vjs_bounds[vi] ) throw( std::runtime_error(std::string("(E) malformed .bones file, a vertex has no parent: ") + std::string(filename) ));
	...
	for(int vi=0;vi<numVertices;++vi) {
		...
		float sumW = 0.;
		while( vj_itr != vj_end ) { sumW += vj_itr->weight; vj_itr++; }

		if( fabs(1.0-sumW) > 0.001 ) throw( std::runtime_error(std::string("(E) malformed .bones file, a vertex has weights that do not sum to 1: ")+filename));
	}
}
