...
bool bvhparse_OFFSET( const std::string& line, float& x, float& y, float& z ) {
	...
	std::string OFFSET;
	ss >> OFFSET;
	if( ss.fail() || OFFSET != "OFFSET" ) return false;
	ss >> x >> y >> z;
	return !(ss.fail());
}
...
bool bvhparse_CHANNELS( const std::string& line, bvhChannelType& type )
{
	...
	std::string CHANNELS;
	ss >> CHANNELS;
	if( ss.fail() || CHANNELS != "CHANNELS") return false;
	int numChans;
	ss >> numChans;
	if( ss.fail() ) return false;
	switch (numChans) {
		case 6 : {
			...
			for( int i=0; i<6; ++i) ss >>channames[i];
			if( !ss.fail() 
		...
		case 3 : {
			...
			for( int i=0; i<3; ++i) ss >>channames[i];
			if( !ss.fail() 
...
bool bvhParse_Frames( const std::string& line, int& numFrames ) 
{
	...
	std::string Frames;
	iss >> Frames;
	if( iss.fail() || Frames != "Frames:") return false;
	iss >> numFrames;
	if( iss.fail() ) { numFrames = 0; return false; }
	return true;
}
...
bool bvhParse_FrameTime( const std::string& line, float& frameTime )
{
	...
	std::string buffer;
	iss >> buffer;
	if( iss.fail() || buffer != "Frame") return false;
	iss >> buffer;
	if( iss.fail() || buffer != "Time:") return false;
	iss >> frameTime;
	if( iss.fail() ) { frameTime = 1.0; return false; }
	return true;
}
...
bool bvhParse_FrameValues( const std::string& line, int numValues, std::vector<float>& values )
{
	...
	std::istringstream iss( line);
	for(int vi=0;vi<numValues;++vi )iss >> values[vi];
  ...
  skel.clear();
	...
	while( true )
	{
		...
		if( parentStack.size() == 1 )
		{
			...
			if( bvhparse_CONSTLINE(line, "MOTION") ) break;
			// ---------------
			// Load the root
			bvhJointType type;
			bvhJoint     joint;
