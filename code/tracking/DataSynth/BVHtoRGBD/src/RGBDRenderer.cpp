...
mGLContext(":1.0")   //TODO Very big todo: take this as an argument from shell $DISPLAY variable
{
  std::cout << "(I): RGBDRenderer constructor called" << std::endl;
  // ---------------------
  // create textures
                              ...
                              RGBDRenderer::LabeledVertex* xs )
{
	...
	float temp[4][3];
	temp[0][0] = -scale; temp[0][1] = -scale; temp[0][2] = noffset;
	temp[1][0] = +scale; temp[1][1] = -scale; temp[1][2] = noffset;
	temp[2][0] = +scale; temp[2][1] = +scale; temp[2][2] = noffset;
	temp[3][0] = -scale; temp[3][1] = +scale; temp[3][2] = noffset;
