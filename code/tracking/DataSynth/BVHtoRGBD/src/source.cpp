...
void buildPMat(float f, int W, int H, float zNear, float zFar, float* PMat){
  PMat[0*4+0] = 2.*f/W; PMat[0*4+1] =  0;      PMat[0*4+2] = 0;                            PMat[0*4+3] = 0;
  PMat[1*4+0] = 0;      PMat[1*4+1] =  2.*f/H; PMat[1*4+2] = 0;                            PMat[1*4+3] = 0;
  PMat[2*4+0] = 0.;     PMat[2*4+1] =  0;      PMat[2*4+2] = (zFar+zNear)/(zFar-zNear);    PMat[2*4+3] = 1; //we ll have to add 1/H and 1/W to get the 1/2 pixel offet later
  PMat[3*4+0] = 0;      PMat[3*4+1] =  0;      PMat[3*4+2] = -(2*zFar*zNear)/(zFar-zNear); PMat[3*4+3] = 0;
}
...
void loadMMat(const std::string& filename, float* MMat){
  ...
  for(int i=0;i<4;++i) {
    for(int j=0;j<4;++j) fin >> MMat[4*i+j];
  }
  if( fin.fail() )  throw( std::runtime_error(std::string("(E) malformed matrix file: ") + filename));
}
                 ...
                 const uint8_t* l)
{
  ...
  for(int y=0;y<h;++y) {
    for(int x=0;x<w;++x) fout<<int(l[w*y+ x])<<" ";
    fout<<"\n";
  }
}
                      ...
                      const uint16_t* d)
{
  ...
  for(int y=0;y<h;++y) {
    for(int x=0;x<w;++x) fout<<d[w*y + x]<<" ";
    fout<<"\n";
  }
}
...
void printGLMat( const float* M, std::ostream& os) {
  ...
  for(int i=0;i<3;++i) {
    os<<"[";
    for(int j=0;j<3;++j) os<<M[4*j+i]<<",";
    os<<M[4*3+i]<<"],\n";
  }
  os<<"[";
  for(int j=0;j<3;++j) os<<M[4*j+3]<<",";
  os<<M[4*3+3]<<"]]\n";
}
...
int main(int argc, char** argv)
{
  ...
  std::cout << "(I): BVH reference loaded" << std::endl;
  ...
  std::cout << "(I): Mesh vectors loaded" << std::endl;

  // -------------------------
  // create the renderer
  RGBDRenderer renderer;
  std::cout << "(I): Renderer created" << std::endl;
  RGBDRenderer::LabeledVertex* vBuffer = new RGBDRenderer::LabeledVertex[X0.size()];
  std::cout << "(I): vBuffer created" << std::endl;
  GLuint*                      iBuffer = new GLuint[3*tris.size()];
  std::cout << "(I): iBuffer created" << std::endl;
  ...
  // copy triangle data
  {
    ...
    for(size_t ti=0;ti<tris.size();++ti ) {
      *t_itr++ = tris[ti].v0; *t_itr++ = tris[ti].v1; *t_itr++ = tris[ti].v2;
    }
  }
  ...
  for(int frameId=0;frameId<firstFrame;++frameId) MFile.readNextFrame(joints_ref, values);
  // -------------------------
  // load the new pose
  for(int frameid=firstFrame;frameid<=lastFrame;++frameid)
  {
    ...
    if( !MFile.readNextFrame(joints_ref, values) ) { std::cout<<"(E) "<<fmotion
    ...
    for(size_t vi=0;vi<X.size();++vi) {
      v_itr->x = X[vi](0); v_itr->y = X[vi](1); v_itr->z = X[vi](2);
